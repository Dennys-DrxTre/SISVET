...
$(function () {
    ...
    getData();
    /** DETAIL DESPARACITACION */
    $('#data tbody').on('click', 'a[rel="detail"]', function () {
        
        modal_title.find('h4').html('Detalle de la Desparacitacion');
        ...
        new_data = document.querySelector('#detail-new_data').textContent = data.new_date;
        $('#Modal_Detail').modal('show');

        /** DETAIL DESPARACITACION EDIT */
        $('#Modal_Detail').on('click', 'a[rel="edit"]', function () {
        
            modal_title.find('h5').html('Editar Desparacitacion');
            $('form')[0].reset();        
    ...
    $('#data tbody').on('click', 'a[rel="pdf"]', function () {
        var tr = tblCate.cell($(this).closest('td, li')).index();
        var data = tblCate.row(tr.row).data();
        window.open('/reports/desparasitacion/detalle/' + data.id + '/');
    });

    /** DELETE DESPARACITACION */
